"""3-Regime Concurrent GPU Benchmark: CUDA Streams, Single Process.

Runs three coupling regimes **concurrently** on a single GPU using
separate CUDA streams, then compares each regime's throughput and
latency against solo baselines to quantify interference.

Architecture
------------
Single Python process, one CUDA context, three CUDA streams:

* ``stream_mf``   : O(N) mean-field,     N = 1,048,576
* ``stream_sk``   : O(N log N) sparse k-NN, N = 65,536
* ``stream_full`` : O(N²) full pairwise,   N = 1,024

Protocol
--------
1. **Solo baselines** — each regime runs alone for 60 s with 30 s telemetry.
2. **Concurrent run** — all three streams fire one step each per loop
...
* NaN detection (especially sparse k-NN *r*)
* Global GPU telemetry time-series

Outputs
-------
* ``Docs/test_and_benchmark_results/concurrent_3regime_benchmark.json``
* ``Docs/test_and_benchmark_results/concurrent_3regime_report.md``
"""
...
# Regime configurations (Goldilocks sizes from prior benchmarks)
...
# ══════════════════════════════════════════════════════════════════
# Concurrent 3-Regime Run (CUDA Streams)
# ══════════════════════════════════════════════════════════════════
...
def run_concurrent(
    ...
    """Run all three regimes concurrently on separate CUDA streams.

    Each loop iteration launches one step of each regime into its own
    stream. Telemetry is collected every *window* seconds.
    ...
    print(f"\n{'═' * 70}")
    print(f"  CONCURRENT 3-REGIME RUN  |  {duration:.0f}s  |  CUDA Streams")
    print(f"{'═' * 70}")
    ...
    stream_sk = torch.cuda.Stream(device)
    ...
    print(
        f"  {'Time':>6s}  {'MF steps':>10s}  {'SK steps':>10s}  "
    ...
    while True:
        ...
        # Sparse k-NN O(N log N)
        ev_sk_s = torch.cuda.Event(enable_timing=True)
        ev_sk_e = torch.cuda.Event(enable_timing=True)
        with torch.cuda.stream(stream_sk):
            ev_sk_s.record(stream_sk)
            current_states["sk"] = models["sk"].step(
                current_states["sk"], dt=DT
            )
            ev_sk_e.record(stream_sk)
        ...
        if window_elapsed >= window or (
            ...
            util_str = f"{gpu_util}%" if gpu_util is not None else "—"
            vram_str = (
                f"{peak_vram_gb:.2f}" if peak_vram_gb else "—"
            )
            print(
                f"  {total_wall_elapsed:>5.0f}s  "
                f"{regime_window_data['mf']['window_steps']:>10,d}  "
                f"{regime_window_data['sk']['window_steps']:>10,d}  "
...
def generate_report(
    ...
    w("# PRINet 3-Regime Concurrent GPU Benchmark (CUDA Streams)")
    w()
    w("## Configuration")
    w()
    w(f"**Generated**: {ts}")
    ...
    w("### Protocol")
    w()
    w("Single Python process, one CUDA context, three streams:")
    w()
    w("| Stream | Regime | Complexity | N |")
    w("|--------|--------|-----------|---|")
    w("| `stream_mf` | Mean-Field | O(N) | 1,048,576 |")
    w("| `stream_sk` | Sparse k-NN | O(N log N) | 65,536 |")
    w("| `stream_full` | Full Pairwise | O(N²) | 1,024 |")
    w()
    w("1. Warm up each regime sequentially (50 steps each)")
    w("2. Launch one step of each regime into its own stream per loop iteration")
    w("3. Synchronize all streams after each iteration for timing")
    ...
    w("### Concurrent Run (5 min, 3 streams)")
    w()
    w("| Metric | O(N) Mean-Field | O(N log N) Sparse k-NN "
      "| O(N²) Full Pairwise |")
    w("|--------|----------------|----------------------"
    ...
    # ── Concurrent Detail ──
    w("## Concurrent Run Detail (5 min, 3 streams)")
    w()
    ...
    w(
        "3. **Stream concurrency effectiveness**: CUDA streams allow "
        "overlapping "
    )
    w(
        "   kernel execution. The degree of actual overlap depends on "
        "SM occupancy; "
    )
    w(
        "   memory-bandwidth-bound kernels (O(N) at 1M) tend to saturate "
        "the memory "
    )
    w("   bus, limiting concurrency benefit for co-running kernels.")
    w()
    ...
    if any_shared_nan:
        w(
            "4. **NaN order parameters detected** in concurrent run — "
            "concurrent "
        )
        w(
            "   stream execution may introduce timing-dependent numerical "
            "issues. "
        )
        w("   Investigate sparse k-NN regime specifically.")
    else:
        w(
            "4. **No new NaN order parameters** introduced by concurrent "
            "execution — "
        )
        w(
            "   numerical stability is preserved under stream concurrency."
        )
    w()

    w(
        "5. **Latency inflation** is the primary cost of concurrency. "
        "Per-step "
    )
    w(
        "   latency increases when multiple workloads compete for GPU "
        "resources, "
    )
    w(
        "   but total throughput (summed across regimes) may still exceed "
        "any "
    )
    w("   single regime's solo throughput.")
    w()
    ...
    w(
        f"6. **Aggregate concurrent throughput**: "
        f"{total_shared:,.0f} osc·s/s total "
    )
    w(
        f"   across all regimes vs {max_solo:,.0f} osc·s/s best solo. "
    ...
    w()
    w(
        f"*Report generated by `concurrent_3regime_benchmark.py` on {ts}*"
    )
...
def main() -> None:
    """Run solo baselines then concurrent 3-regime benchmark."""
    ...
    print(
        "║  PRINet 3-Regime Concurrent GPU Benchmark (CUDA Streams)    ║"
    )
    print(
        "║  O(N) + O(N log N) + O(N²) — Solo + 5 min Concurrent       ║"
    )
    print(
        "╚══════════════════════════════════════════════════════════════╝"
    )
    ...
    meta: dict[str, Any] = {
        "benchmark": "3-Regime Concurrent GPU Benchmark (CUDA Streams)",
        "gpu_name": props.name,
    ...
    # ═══════════════════════════════════════════════════════════════
    print("\n" + "═" * 70)
    print("  PHASE 2: CONCURRENT 3-REGIME RUN (5 min)")
    print("═" * 70)
    ...
    json_path = RESULTS_DIR / "concurrent_3regime_benchmark.json"
    ...
    report_path = RESULTS_DIR / "concurrent_3regime_report.md"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write(report)
    print(f"✓ Report: {report_path}")

    print("\n✓ 3-Regime Concurrent GPU Benchmark complete.")
